...
ovd<-subset(nodal_degreeR2day1, nodal_degreeR2day1$BMI_group == "over")
obd<-subset(nodal_degreeR2day1, nodal_degreeR2day1$BMI_group == "obese")
nod<-subset(nodal_degreeR2day1, nodal_degreeR2day1$BMI_group == "normal")
head(ovd)
...
write.table(ob_rich, "~/Google Drive/MetX/rich_analysis/comb_days/nod_ob_rich.csv", row.names = F, sep=",")

ov_rich<-find_nodes(obd[3:381], 75)
dim(ov_rich)
max(ov_rich$mean_node)
ov_rich
ov_rich<-merge(parcels, ob_rich, by="ROI")
write.table(ov_rich, "~/Google Drive/MetX/rich_analysis/comb_days/nod_ov_rich.csv", row.names = F, sep=",")
